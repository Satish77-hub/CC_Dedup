...
exports.handler = async (event) => {
    try {
        ...
        if (!fileId) return { statusCode: 400, body: JSON.stringify({ message: 'Missing fileId' }) };
        ...
        if (!Item) {
            return { statusCode: 404, body: JSON.stringify({ message: 'File not found or you do not have permission.' }) };
        }
        ...
        return { statusCode: 200, body: JSON.stringify({ downloadUrl: signedUrl }) };
    } catch (error) {
        console.error("Download error:", error);
        return { statusCode: 500, body: JSON.stringify({ message: "Error generating download link.", error: error.message }) };
    }
};